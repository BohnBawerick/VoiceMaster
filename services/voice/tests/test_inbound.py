...
OWNER = "+61491570156"
# Stands in for the AU-region auth token: a real signing key that is NOT the REST token.
REGION_TOKEN = "au-region-token"
...
def test_webhook_accepts_a_regional_signing_token(client, monkeypatch):
    ...
    Our AU number is handled in the AU region, so real inbound webhooks are signed with the
    AU token — never the account's primary/REST token. Validating against the REST token
