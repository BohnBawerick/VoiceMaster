"""Shared builders for the s1 voice-profile tests (Mode C).
...
Mode C's config is module-level constants, so the payload builder goes through
...
def build_payload_and_url(overrides=None, *, outbound=False):
    """One session.update + dial URL through the CURRENT product code, under a
    sandboxed env (server reloaded with ``overrides``, restored afterwards)."""
    import parity_env as pe

    with pe.env_sandbox(overrides) as srv:
        raw = pe.capture_session_raw(srv, outbound=outbound)
        url = srv._realtime_url()
    return raw, url


def effective_config(overrides=None) -> dict:
    """srv._effective_realtime_config() under a sandboxed env."""
    import parity_env as pe

    with pe.env_sandbox(overrides) as srv:
        return srv._effective_realtime_config()
