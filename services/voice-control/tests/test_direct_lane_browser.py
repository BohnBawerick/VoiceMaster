...
def test_one_click_makes_an_agent_hermes_itself(hermes, voice_page, page):
    ...
    page.fill('[data-testid="settings-scribe-language"]', "en-GB")
    page.fill('[data-testid="settings-scribe-keyterms"]', "Hermes, Alex ,  Denver")
    _save(page)
    ...
    assert doc["knobs"] == {"voice": "v-adam", "language": "en-GB",
                            "keyterms": ["Hermes", "Alex", "Denver"]}
...
def test_a_deepgram_option_can_be_cleared_again(hermes, voice_page, page):
    hermes.agent("robot", pipeline="cascade", providers=dict(DIRECT), hermes_profile="vega",
                 knobs={"voice": "v-adam", "language": "en-GB", "keyterms": ["Hermes"],
                        "smart_format": True})
    _with_voices(page)
    page = voice_page("robot")
